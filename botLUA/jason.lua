-- BY : t.me/dev_jason | CH : t.me/devjason
...
if matches[1] == "start" then
	...
	keyboard.inline_keyboard = {		
		 ...
		 {text = "انشاء كود",callback_data='/addcode'},					
   		...
   		{
		 {text = "السورس",url='https://telegram.me/devjason/'},					
		 },
			{
		{text = "Dev Jason", url="https://t.me/devjason"},					
			},		
    	}
...
if msg.cb then 
 ...
 keyboard.inline_keyboard = {		
        {
		{text = "صوره + وصف",callback_data='/photocode'}-- BY : t.me/dev_jason | CH : t.me/devjason,					
		},
        {
...
end  -- BY : t.me/dev_jason | CH : t.me/devjason
if msg.cb then 
 if matches[1] == "photocode" then
   ...
   client:set('jasno'..msg.from.id, 'photocode1')    
  end
  end
...
if matches[1] == 'http'then -- BY : t.me/dev_jason | CH : t.me/devjason,		
if msg.cd then return false end	
...
client:set('list2u'..msg.chat.id, matches[1]..matches[2])
 	keyboard = {} 
	keyboard.inline_keyboard = {		 -- BY : t.me/dev_jason | CH : t.me/devjason,		
    {
		{text = "رجوع ",callback_data='/addcode'},					
		},	
    {
...
client:set('list3u'..msg.chat.id, matches[1]..matches[2])
	...
	keyboard.inline_keyboard = {		
       ...
       {
		{text = "انهاء",callback_data='/getcod'},			 -- BY : t.me/dev_jason | CH : t.me/devjason,				
			},	
	}
 api.sendKeyboard(msg.chat.id, 'يمكنك اضافه المزيد من الازرار او النهاء للحصول على كود', keyboard)  
...
client:set('list3u'..msg.chat.id, matches[1]..matches[2])
 ...
 client:set('jaso'..msg.from.id, 'end')    
   end 
  end -- BY : t.me/dev_jason | CH : t.me/devjason
if msg.cb then 
...
local list2 = client:get('list2'..id) -- BY : t.me/dev_jason | CH : t.me/devjason
...
if list5 then -- BY : t.me/dev_jason | CH : t.me/devjason,		
  ...
  if matches[1] == "start" then
	...
	keyboard.inline_keyboard = {		
			...
			},	
      {
       ...
       {
		{text = "معلومات القنوات",callback_data='/jas'},					
			},	-- BY : t.me/dev_jason | CH : t.me/devjason,		
       {
...
if not msg.cb then -- BY : t.me/dev_jason | CH : t.me/devjason,		
...
if matches[1] ~= 'start' then 
    ...
    if jres.result then 
    text = text..i..' - ['..jres.result.title..'](https://t.me/'..(jres.result.username or 'devjason')..') \n'
end	     
end
end  
    for i=1, #users do
...
if jason == 'addch' then
if msg.cd then return false end		 -- BY : t.me/dev_jason | CH : t.me/devjason
if not msg.forward_from_chat then return false end		
local id = msg.forward_from_chat.id
 local url = 'https://api.telegram.org/bot'..config.bot_api_key..'/getChatMember?chat_id='..id..'&user_id='..msg.from.id..''
 local res = HTTPS.request(url)
...
if matches[1] == 'send' and matches[2] == 'photo' then
    ...
    local users = client:smembers('jason'..bot.id)
    local jjason = client:get('code'..matches[2]..'_'..matches[3])
    local photo = client:get('codep'..matches[2]..'_'..matches[3])
		for i=1, #users do
    local jasonch = client:get('jasonch'..bot.id..''..users[i])
    if jasonch == 'yes' then
    local url = 'https://api.telegram.org/bot'..config.bot_api_key..'/sendPhoto?chat_id='..users[i] ..'&photo='..photo..'&caption='..jjason
    local res = HTTPS.request(url)
    local jres = JSON.decode(res)	
    if not jres.result then    api.sendReply(msg, 'قامه بالمخالفة '..users[1])  end 
	  if jres.result then 
...
if (matches[1] == 'send' and matches[2] == 'mark') then
   ...
   local jasontext = client:get('code'..matches[2]..''..matches[3]..'')
    ...
    local jasonch = client:get('jasonch'..bot.id..''..users[i])
    if jasonch == 'yes' then
    local url = 'https://api.telegram.org/bot'..config.bot_api_key..'/sendMessage?chat_id='..users[i]..'&text='..URL.escape(jasotext)..'&parse_mode=Markdown&disable_web_page_preview=true'
    local res = HTTPS.request(url)
    local jres = JSON.decode(res)	 -- BY : t.me/dev_jason | CH : t.me/devjason
  if not jres.result then    api.sendReply(msg, 'قامه بالمخالفة '..users[1])  end 
	if jres.result then 
    local id = jres.result.message_id
    client:sadd('idjason'..bot.id..''..msg.message_id, '/deleteMessage?chat_id='..users[i]..'&message_id='..id) 
...
if list2 then  -- BY : t.me/dev_jason | CH : t.me/devjason
 keyboard = {}
 	keyboard.inline_keyboard = {		
{{text = list1, url= list1u},},
...
if list4 then 
 keyboard = {}
 	keyboard.inline_keyboard = {		
{{text = list1, url= list1u},}, -- BY : t.me/dev_jason | CH : t.me/devjason
{{text = list2, url= list2u},},	
{{text = list3, url= list3u},},	
...
{{text = list3, url= list3u},},	
{{text = list4, url= list4u},},	
{{text = list5, url= list5u},},	      -- BY : t.me/dev_jason | CH : t.me/devjason  
{{text = "Dev Jason", url="https://t.me/devjason"},},		
}            
 end
 for i=1, #users do
...
if matches[1] == "send" then return false end	 
    client:set('jason'..msg.from.id, 'end')    
		api.sendReply(msg, 'نتضر رجا حتا يتم نشر في القنوات', true)
    local users = client:smembers('jason'..bot.id)
    local jjason = client:get('photo'..msg.from.id)
 for i=1, #users do
 local jasonch = client:get('jasonch'..bot.id..''..users[i])
 if jasonch == 'yes' then
 local url = 'https://api.telegram.org/bot'..config.bot_api_key..'/sendPhoto?chat_id='..users[i] ..'&photo='..msg.photo[3].file_id..'&caption='..jjason
 local res = HTTPS.request(url)
 local jres = JSON.decode(res)	
 if not jres.result then    api.sendReply(msg, 'قامه بالمخالفة '..users[1])  end 
 if jres.result then 
...
if msg.cd then return false end	
if matches[1] == "send" then return false end	
client:set('list3'..bot.id, matches[3]) -- BY : t.me/dev_jason | CH : t.me/devjason
...
{{text = list1, url= list1u},},
{{text = list2, url= list2u},},
{{text = "Dev JasonS", url="https://t.me/devjason"},},		
}       
 end
if list3 ~= "x" then 
 keyboard = {}
...
for i=1, #users do
   ...
   url = url..'&reply_markup='..JSON.encode(keyboard)
   local res = HTTPS.request(url)
   print(res) -- BY : t.me/dev_jason | CH : t.me/devjason
   local jres = JSON.decode(res)	
   if not jres.result then    api.sendReply(msg, 'قامه بالمخالفة  '..users[1])  end 
   if jres.result then 
   local id = jres.result.message_id
...
client:set('jason'..msg.from.id, 'end')    
	...
	for i=1, #users do
    local jasonch = client:get('jasonch'..bot.id..''..users[i])
    if jasonch == 'yes' then -- BY : t.me/dev_jason | CH : t.me/devjason
